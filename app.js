...
function damageCard(index,amount){if(state.phase!==1)return;state.health[index]=Math.max(0,state.health[index]-amount);sound('impact');const destroyed=state.health[index]===0;if(destroyed){explodeCard(index);state.built=Math.max(0,state.built-1);state.combo=1;flashMessage(`POSITION ${String(globalPosition(index)).padStart(2,'0')} DESTROYED · REBUILD`,false);sound('breach')}else if(state.health[index]<=25){flashMessage(`POSITION ${String(globalPosition(index)).padStart(2,'0')} CRITICAL · REPAIR NOW`,false);sound('burn')}renderFortress();updateUI()}
