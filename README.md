...
- Successful sector leaderboards also offer `PRACTICE WEAK CARDS`. This launches an unscored five-card targeting drill selected privately from the player's response history. Practice Raiders reach the floor in approximately 10 seconds. The fortress is repaired, Elite Raiders are disabled, and misses reveal the correct position before returning that card later. Practice continues until every selected card is answered correctly, updates the learning history, then reports time, first-try accuracy, retrained cards, fastest response, and average improvement before offering progression or another drill.
