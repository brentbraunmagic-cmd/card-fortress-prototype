...
- The live score is centered prominently in the top status board at four times the original size, with phase information on the left and compact combo and shield readouts stacked on the right.
...
- A contrasting Restart button occupies the fourteenth value-grid position, directly under 7 and beside King. Restart returns to stack selection and then mode selection. The mode screen includes a Show Tutorial toggle that forces both the build and drone tutorials to replay for that run.
