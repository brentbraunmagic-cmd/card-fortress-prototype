...
- Each recall phase contains one orange-red Elite Raider. A battlefield warning, red perimeter alarm, and three-pulse audible siren activate together above the fortress as it enters, without the damage vibration. It descends 25 percent faster, reaches the floor in approximately 12 seconds, and awards double base interception points. The game stores card response times and mistakes on the device, then selects the Elite from up to three of the player's weakest cards. With no prior history, it selects a random card.
